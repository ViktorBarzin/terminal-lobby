...
func TestPrivReaderReadsAnotherUsersTranscript(t *testing.T) {
	...
	pr := &privReader{osUser: "emo", spawn: inProcessChild(t, home, nil)}
	t.Cleanup(pr.close)
...
func TestPrivReaderRestartsADeadChild(t *testing.T) {
	home, p := mkHome(t, `{"type":"assistant","message":{"content":[{"type":"text","text":"hi"}]}}`)
	var starts int32
	pr := &privReader{osUser: "emo", spawn: inProcessChild(t, home, &starts)}
	t.Cleanup(pr.close)
...
func TestPrivReaderSpawnCommandShape(t *testing.T) {
	got := privopCommand("emo", "/usr/local/bin/session-events")
	want := []string{"sudo", "-n", "-u", "emo", "/usr/local/bin/session-events", "-privop"}
	if len(got) != len(want) {
