...
ssh -o BatchMode=yes "wizard@${DEVVM}" "INCLUDE_TTYD=${TTYD_BIN:+1} STAGE_VAPID=${VAPID_ENV:+1} bash -se" <<'REMOTE'
  ...
  if [[ "${INCLUDE_TTYD:-}" == "1" ]]; then
    ...
    # restart below already covers ttyd, so no extra restart needed. Keep the
    # previous binary aside first: /usr/local/bin/ttyd.prev is the fastest
    # rollback channel (reinstall it + restart ttyd — no rebuild needed).
  ...
  # The lobby page is scripts/deploy-v2.sh's to ship, and only its.
  ...
  for u in ttyd tmux-api clipboard-upload clipboard-cleanup; do
  ...
  sudo systemctl restart ttyd tmux-api clipboard-upload
  sudo systemctl enable --now clipboard-cleanup.timer
  rm -f /tmp/ttyd /tmp/tmux-api /tmp/clipboard-upload /tmp/tmux-attach.sh /tmp/tmux-user-attach /tmp/tmux-user-dirlist /tmp/tmux-user-setfacl /tmp/tmux-restore-user /tmp/tmux-persist-forget /tmp/claude-tmux-state /tmp/claude-se-hook /tmp/show-image /tmp/clipboard-store-clean /tmp/sw.js
  ...
  rm -f /tmp/ttyd.service /tmp/tmux-api.service
...
ssh -o BatchMode=yes "wizard@${DEVVM}" '
  systemctl is-active ttyd tmux-api clipboard-upload
