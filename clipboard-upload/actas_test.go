...
func TestGalleryIgnoresAbsentActAs(t *testing.T) {
	actAsFixture(t)
	for auth, want := range map[string]string{"adminauth": "wizard", "otherauth": "emo"} {
...
// emo simply reads emo's directory — no privilege drop needed, unlike files.
func TestGalleryAdminActsAsAnotherMappedUser(t *testing.T) {
	actAsFixture(t)
	rec := httptest.NewRecorder()
	if got := resolveOSUser(rec, actAsReq("emo", "adminauth")); got != "emo" {
		t.Fatalf("admin ?as=emo resolved %q", got)
	}
}
...
func TestGalleryUnauthenticatedPathIsUnchanged(t *testing.T) {
	actAsFixture(t)
	rec := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodGet, "/list?session=main&as=emo", nil)
