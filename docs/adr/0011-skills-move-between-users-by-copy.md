...
`775`, so emo can already read those files, and wizard reads emo's `700` home
...
- **A live symlink into the owner's home** — always the owner's current version,
  no update tracking, no second copy. It only works one way: emo can read
  `/home/wizard` (`751`), but wizard's own session cannot traverse emo's `700`
  home, so his sessions would fail with `EACCES` on emo's skill. Symmetry would
...
- **Pushing a shared skill into the target's account** — the literal "share with
  emo and it is just there". Rejected because a skill can carry executable
...
- 13 of emo's 22 skill names already exist in wizard's account and 9 of those
...
- A newly installed skill reaches only new sessions. The panel names the sessions
  ...
  that are idle.
- `~/.agents/skills` loses its last consumer. emo's 17 symlinks keep resolving
  and are left alone.
- Retiring the vendored snapshot means the 6 skills only emo has stay available
