...
`id -un = wizard` guard so bob and all other users' SSH/console keep tmux's default (their
...
users keep their path: bob's tmux default-command runs start-claude.sh (with --name); frameArgs
...
overwriting it. Cross-user care: bob's pref was unset (inheriting the app default), so it was
