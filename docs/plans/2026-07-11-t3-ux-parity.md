...
**Multi-user blast radius:** terminal-lobby serves wizard, emo, and anca. Deploy restarts ttyd
...
- [ ] **DEFAULT OFF (challenge finding):** the whole layer ships disabled
  ...
  readline meta-N; emo/anca must see zero change).
...
**Deploy heads-up (emo/anca-visible):** the flip affects ALL coarse-pointer users — at next
...
**Blast-radius reality:** one devvm, one binary, one index.html for wizard/emo/anca; the
...
4. Verify live over SSH: installed stamp `sudo grep -c "$(git rev-parse --short HEAD)" /usr/local/share/ttyd/index.html`;
   authed smoke `curl -s -H "X-authentik-username: vbarzin" localhost:7681/ | grep -o 'tl-build[^"]*'`;
...
7. Post-deploy: full battery §A against production origin (SSH-tunneled authed context) +
   screenshots; store learnings to homelab memory; heads-up note to emo/anca-visible change log
   (font metrics shift cols/rows slightly — intended); candidate ADR-0006 "webfont strategy &
   battery-gated UX changes".

## Rollback

**Channel: SSH (`wizard@10.0.10.10`) — deploy.sh's own transport — NOT the web terminal.**
...
default-command — launcher users (emo's auto-claude) finally get a real shell on demand;
...
FOLLOW-UP same day (deployed as e5f9763) — Viktor: "the nub is yellow-ish when the agent is
completed — make it green." Every theme's --state-done was ALREADY green; the yellow was the
STATE MACHINE: Claude Code's idle notification ("waiting for your input"), sent ~a minute after
a turn finishes, made the claude-tmux-state hook re-promote done→awaiting — so every finished
session quietly turned amber. Classifier fix: permission/approval/"needs your input" wordings
always stamp awaiting (genuinely blocked); the idle-reminder and unknown wordings promote ONLY
from a running turn; a done session stays done (green). Verified all four transitions against a
scratch tmux (done+idle→done, done+permission→awaiting, done+unknown→done,
running+idle→awaiting). Side benefit: finished sessions stop flapping into awaiting, killing a
class of bogus "needs input" pushes.
SECOND FOLLOW-UP (deployed as 9954045) — Viktor: "add on hover effect to explain all the
colors": hovering (or keyboard-focusing) any state nub, the Working note, or the connection
pill on a hover-capable pointer shows a themed legend of the entire color vocabulary — every
state row painted from the live --state-* variables (theme flips honored at show time), the
hovered element's current state highlighted; the pill gets its own connecting/offline pair.
Touch devices keep the plain title tooltips (no hover vocabulary to teach there). Verified both
themes, current-row highlighting, hide on leave/Escape, mobile negative, zero page errors.
Alignment polish (deployed as 3523507, from Viktor's screenshot): a wrapped legend row floated
its dot between the lines — dots now pin to the first text line, wrapped text indents under the
text block, and the awaiting copy is shorter so it rarely wraps.

## Addendum 19 — desktop notifications while focused + push self-heal (2026-07-14, deployed as 92b8625)

Viktor: "i get the in-app notification but nothing on the desktop." A dedicated investigation
agent proved TWO stacked causes with an instrumented-Notification harness (spied constructor +
showNotification, route-fed state edges, zero live writes): (1) PRIMARY — notifyTransitions
returned early on `!away()`, so while the tab was focused NO OS notification could ever fire
(exactly the orchestration posture: watching the lobby while sessions finish; the "in-app"
cue he saw was just the card repainting); (2) SECONDARY — the desktop Web Push endpoint was
registered ONLY on bell-click, never refreshed, so a lapsed/rotated FCM endpoint stayed dead
across reloads while the server 201'd into the void (~140 accepted sends to his desktop
endpoint on the day of the report, zero rendered). The server-side activity gate and 4xx were
ruled out with journal evidence. FIX: the away gate became PER-SESSION — the session on screen
stays quiet while focused (you are watching it), everything else (hidden, unfocused, or a
BACKGROUND session while focused) raises the OS notification; the shared tl-<session> tag
coalesces with background push so nothing double-alerts. And subscribePush self-heals on every
load when the bell is on + permission granted (idempotent: getSubscription reuse, upsert by
endpoint). Verified matrix: focused+active 0, focused+non-active 1 (the fix), hidden 1,
visible-unfocused 1; subscription attempts observed at boot in every scenario. REMAINING
USER-SIDE CHECK if the desktop stays silent after a reload: the OS layer — macOS/Windows
notification settings for the browser (Focus/DND, banners disabled) — and Settings →
Notifications "Subscribed here" readout distinguishes a stale endpoint from OS suppression.

## Addendum 20 — awaiting goes purple (2026-07-14, deployed as d617e6e)

Viktor: "change the amber to purple — similarly to t3 code. that looks nicer." Every theme's
--state-awaiting became a palette-tuned violet: GitHub purple #a371f7 (slate), violet-500/600
(the T3 pair), Catppuccin mauve (mocha/latte), IBM Carbon purple (carbon), muted lavender
(mono), deep purple (ink). Everything reading the variable follows automatically (state dots,
title badge, favicon badge, legend swatches); the connection pill's hardcoded amber became
var(--state-awaiting); the hover legend stopped naming colors in WORDS (the swatch shows the
truth, and words break on restyles — this very lesson). Toast WARNINGS stay amber
deliberately: warning and awaiting are different vocabularies. Verified across five themes +
legend swatch + pill variable, zero page errors.

## Addendum 21 — self-refreshing clients (2026-07-14, deployed as e09d8ea)

Viktor: "as soon as we push new changes, they are pushed to clients. i think there's a library
for this." Research (online + code) concluded NO library earns its place — an architectural
dead-end here, not a preference: deploy.sh ships sw.js VERBATIM (only index.html gets the
build stamp) and the SW is push-only with no fetch handler by design, so workbox-window /
vite-plugin-pwa update machinery would literally never fire and couldn't change what the page
runs if it did. The actual gap: the shipped stale-tab healer only ran on WS reconnect —
terminal pages caught deploys (ttyd restart drops their socket) but the LOBBY/top shell has no
WS and never checked. AS-BUILT (healer-only; the interim X-TL-Build header fast-path was
dropped when its latency rationale evaporated): the lobby re-hashes its own bytes every 5s via
conditional GET (the patched ttyd serves a strong ETag — unchanged is a bodyless ~150-byte
304) plus on visibilitychange/pageshow (iOS resume); a change reloads the TOP exactly once —
the embedded terminal posts tl-build-stale upward and never self-reloads when framed (an
adversarial challenger PROVED the symmetric-lock alternative left the lobby stale while
double-reloading the terminal); typing gets a deferred tappable "Update ready" pill (reusing
the kb-activity signal); the baseline re-arms from the NEW page so loops are impossible. GATE
(3 workflow agents, all APPROVE): independent validator reproduced 4.5s detection, exactly-one
reload with the storm guard deliberately CLEARED, driven §A.5 green; steady-state challenger
failed to produce any loop across a 14-min soak (218 ticks, zero reloads), 3× ttyd
kill/restart churn, a 50× spoof-flood collapsed to one reload, and origin-checked message
handlers; red-line challenger confirmed input surfaces byte-identical. Advisory findings
recorded, none blocking: backend-only deploys still cause one bounded client reload (deploy.sh
stamps HEAD each deploy — acceptable, arguably right); the update pill's tap silently defers
inside the 2-min storm window; a dismissed pill doesn't re-raise that page-life (idle/hidden
reload still lands). Process note: the original implementation workflow was stopped mid-run
when the design pivoted, its orphaned implementer died mid-edit, and the research agent
absorbed the remains as sole writer — the landing is a single squash commit. Also caught
during build: an in-flight-fetch guard was added then REMOVED because a backgrounded iOS
fetch would wedge the healer permanently on resume.

## Addendum 19b — per-device notification test (2026-07-14, deployed as e8f600c)

Viktor after 19: "desktop notifications still don't work. also, it says it sends notification
to 3 devices — why?" The old Test button ONLY exercised the server fan-out (all registered
devices — his two iPhone endpoints + the desktop), which both surprised him and cannot isolate
one device: the server's 201s prove acceptance, never rendering. Settings now has TWO tests:
"Test this device" fires a LOCAL notification (no server, no phones) — exercising exactly the
browser→OS display chain on that machine; if nothing appears while permission is granted, the
OS or browser is suppressing the site (macOS Focus/DND, Chrome banners off) and the toast says
so. "Test all devices" keeps the fan-out with honest copy. The fan-out on REAL alerts stays by
design (a finishing session should reach the phone even with the desktop closed); only the
TEST needed scoping. Verified: local test records exactly one notification with zero server
requests.

## Addendum 22 — native desktop wheel scrolling (2026-07-14, deployed as efbb07d)

Viktor (desktop, Mac): "make the scrolling experience more native — right now we seem to be
using the tmux one and that's fairly choppy." Investigated before touching this surface (a past
mouse-mode change broke his scroll/selection workflow — memory 8388, the app's most guarded
path). MEASURED root cause of trackpad history scrolling: three quantizers stack — xterm 6
damps sub-50px pixel wheels to ×0.3 (a smooth trackpad stream loses ~85-90%), mouse-tracking
panes are capped at one scroll report per DOM wheel event, and tmux copy-mode jumps a fixed 5
lines per surviving report. True pixel-smooth is IMPOSSIBLE under tmux's whole-row alt-screen
redraw; the achievable "native" is de-damped + proportional + frame-paced one-row LINE motion
riding the macOS inertia tail. Two AskUserQuestion rounds pinned it: trackpad + terminal-history
surface + interceptor default-ON, and (blast-radius surfaced mid-build) the tmux half scoped to
wizard only. SHIPPED: (1) frontend — an xterm attachCustomWheelEventHandler (gestures.wheelSmooth,
default on, opt-out) capturing the full trackpad travel and re-emitting rAF-paced one-row
line-wheels via the existing emitLineWheel; OFF detaches for byte-identical raw passthrough (the
instant-revert safety net that makes default-on acceptable here); isTrusted guard, modified/
horizontal passthrough, gesture master-kill; wheelSpeed 1/1.5/2/3× tunable. (2) tmux — copy-mode
WheelUp/Down scroll 1 line instead of the default 5 so the emits map 1:1, scoped by an if-shell
`id -un = wizard` guard so emo and all other users' SSH/console keep tmux's default (their
per-user servers untouched); applies to tmux servers started after deploy. GATE (adversarial
workflow): validator + red-line challenger APPROVED; the steady-state challenger BLOCKED on a
runaway coast (the wheel accumulator was uncapped → a fast flick / coalesced delta / the inertia
tail backlogged and emitted for multiple seconds after the fingers stopped, 2187 post-stop emits
at speed 3 — and there is NO JS momentum by design). Fixed by clamping the accumulator to one
frame's drain (SCROLL_MAX_EVENTS_PER_FEED × rowPx); an independent re-verify confirmed post-stop
emits 2187→0 across all speeds/directions with de-damping byte-identical, no rAF spin, no
re-entry. DEVICE-MANUAL for Viktor: confirm the trackpad feel; flip Settings → smooth-scroll OFF
instantly if it ever regresses; tune wheelSpeed; an ALREADY-RUNNING tmux server needs a restart
(kill-server / reboot) to pick up the 1-line copy-mode — new servers get it automatically. 

## Addendum 23 — reconnect no longer flickers an active terminal (2026-07-14, deployed as 957b67c)

Viktor: "the screen flickers on reconnect. keep it connected and repaint only on refresh." Root
cause (measured against the just-shipped self-refresh): every deploy restarts ttyd, dropping
each client's WebSocket; the terminal reconnects, and on a changed build the update policy
reloaded the whole page UNLESS the user had typed in the last 10s. A reader watching a session
therefore got a full white page-reload flash on every deploy (~25 today). New policy in
requestTopReload: while a terminal is ATTACHED and the tab is VISIBLE, a new build NEVER
auto-reloads — it surfaces the sticky "Update ready" pill and waits for the tap (or the tab
hiding / idle path). Auto-reload stays immediate only with no attached terminal (pure lobby)
or a hidden tab — nothing on screen to interrupt. The now-unused typing-recency machinery
(iframe observe-only tl-kb-activity keydown emit + parent handler + frameTypingRecently) is
retired — no input-path change, net −25 lines. Verified in the harness: attached+visible + new
build → no reload + pill; no-terminal → reloads. The residual reconnect blip (tmux redrawing on
WS re-attach) is minor/server-driven; the dominant full-page flash is gone. SCROLL note (same
message): the scroll code (addenda 16/22) is confirmed LIVE + enabled and works for
shell-history / copy-mode; inside Claude Code (or any mouse-tracking TUI) the wheel/swipe is
forwarded to the app and tmux never enters copy-mode, so the terminal layer cannot change
scrolling there (memory 7320) — the likely reason "nothing changed" if scrolling inside Claude
Code; pending a plain-shell scrollback confirmation from Viktor. 

## Addendum 24 — new-session dropdown defaults to claude, "Default" removed (2026-07-15, deployed as b7341e3)

Viktor: "remove the default option and set the default to claude." The "Default" choice meant
"use the account's tmux default-command" — which for a launcher-less account (wizard) is a plain
shell, a confusing thing to default to when the point is to start Claude. Dropdown is now
claude / codex / shell, defaulting to claude (PREF_DEFAULTS.session.newCommand 'default' →
'claude'). 'default' remains a VALID backing value (not shown in the dropdown) so launcher
users keep their path: emo's tmux default-command runs start-claude.sh (with --name); frameArgs
appends the command key ONLY when it isn't 'default', so a 'default' pref still yields the bare
launcher attach. syncNewCmd reflects a 'default' pref as the visible "claude" without
overwriting it. Cross-user care: emo's pref was unset (inheriting the app default), so it was
pinned to 'default' out-of-band to preserve his launcher — unset would otherwise now resolve to
claude and bypass it. (Wizard's 'claude' also carries his personal flags via
~/.config/terminal-lobby/commands: --dangerously-skip-permissions --model claude-opus-4-8
--effort max, from his history.)
