...
- [ ] **Step 2: Write `main.go`.** Single `httputil.ReverseProxy`-based router (Go ≥1.21 handles
  ...
  -tmux-api 127.0.0.1:7696 -clipboard 127.0.0.1:7695 -user vbarzin`. Route by the table from
...
4. Verify live: installed stamp `grep -o 'tl-build[^,]*' /usr/local/share/ttyd/index.html` (or
   ...
   `curl -s -H "X-authentik-username: vbarzin" localhost:7681/ | grep build`; watch
