...
- [ ] **Step 6: Battery line.** Append to BATTERY.md §B: in-page
  ...
  metrics check (JBM may be wider than the OS fallback): at a 390×844 mobile viewport record
  cols×rows before/after — cols must stay ≥80 portrait; box-drawing alignment intact at both
...
- [ ] **(TDD)** Tests first: GET `/manifest.webmanifest` → 200 `application/manifest+json` +
  ...
  `/icon-192.png`, `/icon-512.png` (`image/png`) and `/fonts/<each vendored woff2>`
...
**Stage B — after Wave 3-4 gates green (ttyd patch, flow control, PWA, keybindings):**
5. `scripts/build-ttyd.sh` (preflight build deps first) → `out/ttyd` present; merge + push Wave
   3-4 commits; presence claim; `./scripts/deploy.sh`; same verification as step 4 PLUS
   flowprobe against :7681 (authed) and the un-paused throughput control.
