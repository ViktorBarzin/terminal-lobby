...
devvm (`10.0.10.10`). The four Services in `infra/stacks/terminal` are manual
...
flowchart LR
  ...
  subgraph DEVVM["devvm 10.0.10.10 — immovable"]
...
- That preference was recorded for the `learn`/`plans` pod — a *pure* static site
  ...
  serving path while every dynamic request still lands on `10.0.10.10`. The
