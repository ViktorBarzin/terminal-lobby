...
sequenceDiagram
  ...
  alt session exists
    API-->>TTYD: {"mode":"rw"}
    TTYD->>TMUX: sudo -n -H -u emo tmux attach-session -t name
  else session does not exist
    API-->>TTYD: {"mode":"rw","create":true}
    TTYD->>TMUX: sudo -n -H -u emo tmux-user-attach name dir cmd
  end
```
...
- **Every switch is recorded.** A journal line plus an `admin.actas` telemetry
  event carrying actor, target and session, so "was anyone in emo's account on
...
| wizard `?as=emo` → open a session | attaches read-write as emo; `id -un` says `emo` |
| wizard `?as=emo` → new session | created under emo's uid |
