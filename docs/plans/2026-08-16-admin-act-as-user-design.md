...
- **The choice is remembered under the target.** The Watch choice was keyed by
  ...
  driving emo's `code` decides nothing about yours. Session names are bounded to
...
opens — including one a third party shared with emo read-write, since that grant
...
| wizard `?as=emo` → Watch → take control | re-attaches read-write; `client_readonly=0`, the audit line says `DRIVING (read-write)` (2026-08-21) |
| …then reopen that session | comes up driving (stored under `as:emo:`); every OTHER session still comes up watching (2026-08-21) |
| wizard `?as=emo` → a session that is not running | the attach fails; nothing is created in emo's account (2026-08-17) |
| wizard `?as=emo` → the Watch control | on, live, tooltip names emo in both states (2026-08-21) |
