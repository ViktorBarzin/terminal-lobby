...
- [ ] **Step 1: Write the failing test** — table test: `mapUser("vbarzin")=="wizard"`, unknown → error, `"vbarzin@meta.com"` strips at `@`. (Mirror `tmux-api`'s existing map parser; read `tmux-api/main.go` for the exact format `<auth_local>=<os_user>[:<cwd>]`.)
