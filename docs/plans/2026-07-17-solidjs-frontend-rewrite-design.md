...
- A **second real user (`emo`)** shares this daily driver — a regression hits them
  too.
...
2. **Canary soak before cutover.** Deploy the rewrite to a **parallel host**
   ...
   `infra/stacks/terminal/`) and dogfood it — Viktor **and emo** — against the real
...
3. **`emo` in the acceptance gate**, not just Viktor.
...
- **P5 — Mobile.** Soft-key toolbar, latching modifiers, gestures (pinch-font;
  ...
  probes.
- **P6 — Canary soak + cutover.** Parallel-host soak (§5.2) with Viktor + emo; full
...
- **Golden-master (blocking, P0.5):** Playwright+pytest over `dev-harness.py` — the
  real cutover gate.
- **Manual (BATTERY.md):** per-feature acceptance, **run by Viktor and emo**; 4
...
- **Challenger B (strategy/risk) — REVISE.** Caught: the retracted SW-cache claim;
  ...
  d; 3 field-found ADR-0003 addenda, none test-caught); missed blast radius (emo,
