...
- **Deploy follow-ups (before M3 multi-user):** (a) `emo` locked out — `/etc/ttyd-user-map` key `emil.barzin` has a dot the validator rejects; needs a dotless roster/map alias (nothing reached emo — isolation held). (b) `acd-demux` build-info reports `(uncommitted)` — stamp the build to its commit. (c) firewall `:7690/:7691` to the k8s node IPs (network-isolation layer; the `X-Gateway-Auth` secret is the control until then).
