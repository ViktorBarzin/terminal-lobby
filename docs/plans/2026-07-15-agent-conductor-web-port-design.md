...
- **Deploy follow-ups (before M3 multi-user):** (a) `bob` locked out — `acd-demux`'s `valid_name` rejects the dot in the Authentik local-part `bob.smith`, which the **existing ttyd path** (`tmux-attach.sh`, memory #8112) accepts and routes fine — so it's an **AC-validator divergence**, not a roster bug. Fix in `ac-gateway/src/map.rs`: allow `.` in the Authentik-local-part validator while keeping the os_user path-component check strict/dotless (they currently share one `valid_name`). Nothing reached bob — isolation held. (b) `acd-demux` build-info reports `(uncommitted)` — stamp the build to its commit. (c) firewall `:7690/:7691` to the k8s node IPs (network-isolation layer; the `X-Gateway-Auth` secret is the control until then).
