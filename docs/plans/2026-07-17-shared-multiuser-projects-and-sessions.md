...
**End-to-end live proof (deployed stack):** a real read-only cross-user attach — `bob` authorized to attach `wizard`'s session, a real tmux client connected `readonly=YES` with `-r` sourced server-side, the client tty recorded for kick, revoke clean; co-ownership `getfacl` showed `bob` gaining then losing `rwX`; wrapper guardrails refuse home-root / outside-home / bad-user; existing lobby unaffected (zero foreign sessions for real users). Backend `go test`/`vet` green throughout.
...
```mermaid
sequenceDiagram
    participant G as Guest browser (auth: bob.smith)
    ...
    TT->>A: args + X-authentik-username: bob.smith
    A->>A: guest OS user = bob (resolveOSUser)
    A->>ST: bob allowed to attach (owner=wizard, name)? mode?
    alt authorized
        ST-->>A: yes, mode=ro
        A->>ST: record bob -> client_tty (this pty)
...
- **Attach-as-owner** overrides the header-derived OS user with the target `owner`, *only after* the wizard-owned share table authorizes this guest for `(owner, name)`. No new sudo grant — the existing broad `(bob)/(carol) NOPASSWD: /usr/bin/tmux` covers `attach-session`.
...
- **Trust posture:** a 2–3 person box of trusted users (wizard, bob, carol per memory #7290). The design leans on that throughout; it is **not** safe for untrusted tenants.
...
- `carol` → `/etc/ttyd-user-map` line to participate (already in the sudoers grant). Design is N-user generic.
...
- Live E2E on the devvm (the 2026-07-16 lesson — verify the *attached session*, not just the modal): bob attaches a wizard session RO (input rejected; roam disclosed) and RW (drives it, shell as wizard); revoke kicks the exact guest client; a shared project shows both users' sessions; a guest creates a session in a shared co-owned dir and writes a file both can edit; unshare strips ACLs. Confirm each attached session's `pwd`, identity, and RO-enforcement.
