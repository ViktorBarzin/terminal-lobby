...
reading — the description, the file and executable counts, why an update is
offered. The two lists were inconsistent for an afternoon: a plugin acted inline
while a skill made you expand it first. Under 560px the row wraps and the buttons
take their own line rather than shrinking, because a Delete squeezed to an
ellipsis is worse than a taller row.
