...
`emo` and `ancamilea` also have sessions on this box.
...
box-wide prompt flood back for every user including emo.
...
| 3 | Agents drive the **real deployed artifact against the real backends**, locally | Claude Code runs *on* the devvm (`10.0.10.10`); all six ports are on localhost. No tunnel, no Authentik hop |
