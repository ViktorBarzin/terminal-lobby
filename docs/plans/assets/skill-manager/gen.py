...
# =============================================================================
def m1():
    s = Svg(W, 660, "Skills group in the lobby Settings panel")
    s.panel()
    # header
    s.text(PAD, 34, "Settings", size=15, weight="600")
    s.text(W - PAD - 4, 35, "✕", size=15, fill=MUTED, anchor="end")
    # a neighbour group for context
    y = 62
    s.group_label(y, "Theme")
    for i in range(3):
        s.rect(PAD + i * 68, y + 10, 62, 26, fill=BG_PAGE,
               stroke=ACCENT if i == 1 else BORDER, rx=R)
        s.text(PAD + i * 68 + 31, y + 27, ["Carbon", "Slate", "Mono"][i], size=11,
               fill=FG if i == 1 else MUTED, anchor="middle")
    s.text(PAD + 3 * 68 + 6, y + 27, "…", size=12, fill=MUTED)
    y += 52
    s.divider(y)
    ...
    def row(y, name, meta, *, on=None, install=False, warn=False, mono=True,
            metafill=MUTED, badge=None):
        if on is not None:
            s.check(PAD, y, on)
        s.text(PAD + (22 if on is not None else 0), y + 4, name, size=13,
               font=MONO if mono else UI, fill=FG if on is not False else MUTED,
               op=None if on is not False else 0.75)
        if badge:
            bx = PAD + (22 if on is not None else 0) + len(name) * 7.3 + 8
            s.rect(bx, y - 7, len(badge) * 5.6 + 12, 15, fill="#2a1f16" if warn else BG_PAGE,
                   stroke=DANGER if warn else BORDER, rx=7)
            s.text(bx + (len(badge) * 5.6 + 12) / 2, y + 4, badge, size=10,
                   fill=DANGER if warn else MUTED, anchor="middle")
        if install:
            s.btn(W - PAD - 66, y, "Install", w=66, kind="accent")
        elif meta:
            s.text(W - PAD, y + 4, meta, size=11, fill=metafill, anchor="end")

    y += 20
    s.text(PAD, y, "Mine (38)", size=12, fill=FG, weight="600")
    y += 22
    row(y, "grilling", "own", on=True); y += 26
    row(y, "publish-page", "own", on=True); y += 26
    row(y, "cluster-health", "from emo · ⟳ update", on=True, metafill=ACCENT); y += 26
    row(y, "caveman", "from emo", on=False); y += 26
    s.text(PAD + 22, y + 4, "…  34 more", size=11, fill=MUTED, font=UI)

    y += 24
    s.divider(y)
    y += 22
    s.text(PAD, y, "Plugins (7)", size=12, fill=FG, weight="600")
    y += 22
    row(y, "superpowers", "5.1.0 · ⟳ 5.3.0", on=True, metafill=ACCENT); y += 26
    row(y, "context7", "61c059", on=True); y += 26
    row(y, "gopls-lsp", "1.0.0", on=False); y += 26
    s.text(PAD + 22, y + 4, "…  4 more", size=11, fill=MUTED, font=UI)

    y += 24
    s.divider(y)
    y += 22
    s.text(PAD, y, "From emo — not installed (8)", size=12, fill=FG, weight="600")
    y += 24
    row(y, "diagnose", None, install=True); y += 30
    row(y, "write-a-skill", None, install=True); y += 30
    row(y, "tdd", None, warn=True, badge="differs", install=True); y += 30
    row(y, "file-issue", "= same as yours"); y += 26
    s.text(PAD, y + 4, "…  4 more", size=11, fill=MUTED, font=UI)

    y += 26
    s.text(PAD, y, "Everyone on this box can see everyone's skills. Installing copies",
           size=11, fill=MUTED)
    s.text(PAD, y + 15, "the skill into your account; the owner's copy is untouched.",
           size=11, fill=MUTED)
    s.h = y + 34
    s.p.insert(0, "")            # keep panel first
    s.p[0] = ""
    return s
...
def m3():
    ...
    s.panel()
    y = 34
    s.group_label(y, "From emo — not installed")
    y += 26
    ...
    s.text(PAD, y + 19, "emo's is written. Identical skills show \"= same as yours\"", size=11, fill=MUTED)
...
def m4():
    ...
    s.text(PAD + 24, y, "installed diagnose from emo", size=13)
    y += 24
...
for name, fn in (("m1-skills-group.svg", m1), ("m2-row-expanded.svg", m2),
    ...
    s.p.insert(0, f'<rect x="1" y="1" width="{W-2}" height="{s.h-2}" rx="{RCARD}" '
                  f'fill="{BG_CARD}" stroke="{BORDER_STRONG}" stroke-width="1"/>')
    s.out(name)
    print(f"{name}  {W}x{s.h}")
