...
`192.0.2.10:{7681,7683,7684,7685,7686,7688}`, the IngressRoutes and the
...
Prometheus already scrapes this box as `job=devvm` at `192.0.2.10:9100`, so
...
for `bob`. Restart-if-changed keeps that to the units that actually moved, and
...
- **Whether Woodpecker pods can actually open TCP 22 to `192.0.2.10`.** Routing
...
- **Where `carol` sits.** She has a home directory and no row in
