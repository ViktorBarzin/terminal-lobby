...
| Picker reach | **All 3 users** | wizard runs the scan directly; bob/carol via an audited sudo wrapper. |
...
- **devvm:** `tmux-user-dirlist` — audited, argument-free wrapper (`fd --type d
  ...
  bob + carol (also closes carol's previously-missing base grant).
...
- Live on the devvm: `GET /dirs` = 1251 dirs for wizard, **200 + 265 dirs for bob via sudo**.
...
the tmux `default-command`" — correct for LAUNCHER users: bob and carol both
have `~/start-claude.sh`, bob wires it via `set -g default-command`, and bob's
...
> silently blocked by `/home/bob` permissions — that no launcher users remained.
> It was briefly deployed, then reverted before bob loaded it: the heal would
> have clobbered bob's pinned `'default'` and bypassed the launcher. `'default'`
