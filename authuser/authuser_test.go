...
func TestNoActAsParamResolvesToTheCaller(t *testing.T) {
	g := gateWith(t, "wizard\n")
	for _, caller := range []string{"wizard", "bob"} {
...
func TestAdminMayActAsAnotherMappedUser(t *testing.T) {
	g := gateWith(t, "wizard\n")
	got, err := g.Effective("wizard", "bob", mapped)
	if err != nil {
		t.Fatalf("admin acting as bob: unexpected error %v", err)
	}
	if got != "bob" {
		t.Fatalf("admin acting as bob resolved to %q, want bob", got)
	}
}
...
func TestNonAdminMayNotActAsAnyoneElse(t *testing.T) {
	g := gateWith(t, "wizard\n")
	_, err := g.Effective("bob", "wizard", mapped)
	if !errors.Is(err, ErrNotAdmin) {
		t.Fatalf("bob acting as wizard: got %v, want ErrNotAdmin", err)
	}
	// ...including toward a third party, not just toward the admin.
	if _, err := g.Effective("bob", "carol", mapped); !errors.Is(err, ErrNotAdmin) {
		t.Fatalf("bob acting as carol: got %v, want ErrNotAdmin", err)
	}
}
...
// "-bob" above is refused by the charset, but the empty string is the one that
...
func TestEmptyActAsIsNotARequest(t *testing.T) {
	g := gateWith(t, "wizard\n")
	got, err := g.Effective("bob", "", mapped)
	if err != nil {
		t.Fatalf("empty ?as= for a non-admin: unexpected error %v", err)
	}
	if got != "bob" {
		t.Fatalf("empty ?as= resolved to %q, want bob", got)
	}
}
...
func TestMissingAdminsFileFailsClosed(t *testing.T) {
	...
	if _, err := g.Effective("wizard", "bob", mapped); !errors.Is(err, ErrNotAdmin) {
...
func TestAdminsFileParsing(t *testing.T) {
	...
	for _, u := range []string{"wizard", "carol"} {
		if !g.IsAdmin(u) {
	...
	if g.IsAdmin("bob") {
		t.Fatal("bob is commented out and must not be an admin")
	}
	if g.IsAdmin("") {
...
func TestGateIsInertWithoutTheParameter(t *testing.T) {
	for _, contents := range []string{"", "wizard\n", "bob\n"} {
		...
		for _, caller := range []string{"wizard", "bob", "carol"} {
			got, err := g.Effective(caller, "", mapped)
