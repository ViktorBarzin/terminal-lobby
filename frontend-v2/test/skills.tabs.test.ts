...
describe("tabsFor", () => {
  it("puts the caller first, then each account, then plugins and sessions", () => {
    ...
    expect(tabs.map((t) => t.id)).toEqual(["mine", "peer:emo", "plugins", "sessions"]);
    expect(tabs.map((t) => t.label)).toEqual(["Mine", "emo", "Plugins", "Sessions"]);
  });

  it("counts a peer tab by what is takeable, not by what they have", () => {
    // Three skills, but the identical one is not takeable.
    const emo = tabsFor(inv())[1]!;
    expect(emo.count).toBe(2);
  });

  it("keeps a tab for an account it could not read, with no count", () => {
    const tabs = tabsFor(inv({ peers: [{ user: "emo", unreachable: true }] }));
    expect(tabs.map((t) => t.id)).toContain("peer:emo");
    expect(tabs.find((t) => t.id === "peer:emo")!.count).toBe(0);
  });
...
describe("resolveTab", () => {
  ...
  it("falls back to the first when the selection is gone", () => {
    ...
    expect(resolveTab(tabs, "peer:emo")).toBe("mine");
  });
...
describe("the filter", () => {
  ...
  it("applies to each list", () => {
    ...
    expect(peerRows(inv(), "emo", "diag").skills.map((s) => s.name)).toEqual(["diagnose"]);
  ...
  it("returns the peer's block so the caller can tell empty from unreachable", () => {
    const { block } = peerRows(inv({ peers: [{ user: "emo", unreachable: true }] }), "emo", "");
