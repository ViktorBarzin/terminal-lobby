...
export type MirrorEvent =
   ...
   *                  called by SessionView's `sendBytesToPty` for the soft keys and
   ...
   *   :9689          SessionView's `sendBytesToPty`, so they arrive through that same
   *                  bridge.
   *   :10293         a fresh socket attach, in `attach.ts`.
   *
   * The bridge is the load-bearing one: it calls the attachment's `send`
   * directly (`attach.ts`'s returned `send`), so xterm's onData never sees
   * those bytes and
