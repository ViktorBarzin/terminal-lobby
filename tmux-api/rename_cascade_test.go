...
func TestRenameCarriesIntoTheProjectStore(t *testing.T) {
	...
	if err := ps.update(func(set *ProjectSet) error {
		set.Projects = []GlobalProject{{
			ID: "p1", Name: "tripit", Members: []Member{{OSUser: "wizard"}, {OSUser: "emo"}},
			Sessions: []SessionRef{
				{Owner: "wizard", Name: "deploy-the-thing"},
				{Owner: "emo", Name: "deploy-the-thing"}, // same name, different owner
	...
	want := []SessionRef{
		...
		{Owner: "emo", Name: "deploy-the-thing"}, // another user's session is not ours to rename
		{Owner: "wizard", Name: "untouched"},
	}
...
func TestRenameCarriesIntoTheShareStore(t *testing.T) {
	...
	if err := ss.update(func(set *ShareSet) error {
		set.Shares = []Share{
			{Owner: "wizard", Name: "deploy-the-thing", Guest: "emo", Mode: "ro"},
			{Owner: "wizard", Name: "deploy-the-thing", Guest: "ancamilea", Mode: "rw"},
			{Owner: "emo", Name: "deploy-the-thing", Guest: "wizard", Mode: "ro"},
			{Owner: "wizard", Name: "untouched", Guest: "emo", Mode: "ro"},
		}
		return nil
	}); err != nil {
	...
	for _, want := range []Share{
		{Owner: "wizard", Name: "fix-the-parser", Guest: "emo", Mode: "ro"},
		{Owner: "wizard", Name: "fix-the-parser", Guest: "ancamilea", Mode: "rw"},
		{Owner: "emo", Name: "deploy-the-thing", Guest: "wizard", Mode: "ro"},
		{Owner: "wizard", Name: "untouched", Guest: "emo", Mode: "ro"},
	} {
		found := false
...
func TestRenameCarriesOnWhenOneStoreFails(t *testing.T) {
	...
	ss := shareStoreInstance
	if err := ss.update(func(set *ShareSet) error {
		set.Shares = []Share{{Owner: "wizard", Name: "deploy-the-thing", Guest: "emo", Mode: "ro"}}
		return nil
	}); err != nil {
		t.Fatal(err)
	}
