...
func TestPackThenUnpackReproducesTheSkill(t *testing.T) {
	...
	backup, err := Unpack(home, "diagnose", "emo", blobs, st.Hash, false, at)
	if err != nil {
		t.Fatal(err)
	}
	if backup != "" {
	...
	if p.From != "emo" || p.SourceHash != st.Hash || p.InstalledAt != "2026-08-19T09:12:00Z" {
...
func TestUnpackRefusesAnExistingSkillUnlessReplacing(t *testing.T) {
	...
	if _, err := Unpack(home, "tdd", "emo", blobs, st.Hash, false, at); err == nil {
	...
	backup, err := Unpack(home, "tdd", "emo", blobs, st.Hash, true, at)
	if err != nil {
		t.Fatal(err)
	}
	if backup == "" {
...
func TestUnpackRejectsBlobsThatDoNotDescribeASkill(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if _, err := Unpack(home, "x", "emo", c.blobs, "sha256:whatever", false, at); err == nil {
...
func TestUnpackRefusesWhenTheContentDoesNotMatchTheDeclaredHash(t *testing.T) {
	...
	if _, err := Unpack(home, "x", "emo", blobs, "sha256:0000", false, time.Now().UTC()); err == nil {
...
func TestUnpackHonoursTheSizeAndCountLimits(t *testing.T) {
	...
	if _, err := unpackWith(home, "x", "emo", big, "sha256:x", false, time.Now().UTC(),
	...
	if _, err := unpackWith(home, "x", "emo", many, "sha256:x", false, time.Now().UTC(),
