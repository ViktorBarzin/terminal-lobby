...
func TestManifestRoundTripsAndOmitsNothing(t *testing.T) {
	...
	m.Record("diagnose", "bob", "sha256:abc", at)
	...
	if p.From != "bob" || p.SourceHash != "sha256:abc" || p.InstalledAt != "2026-08-19T09:12:00Z" {
...
func TestManifestLivesBesideTheSkillsAndNotInsideOne(t *testing.T) {
	...
	m.Record("x", "bob", "sha256:1", time.Now().UTC())
	if err := m.Save(home); err != nil {
		t.Fatal(err)
	}
...
func TestScanClassifiesOwnInstalledDisabledAndModified(t *testing.T) {
	...
	at := time.Date(2026, 8, 19, 9, 12, 0, 0, time.UTC)
	m, _ := LoadManifest(home)
	m.Record("diagnose", "bob", mustHash(t, installed), at)
	m.Record("teach", "bob", "sha256:stale-on-purpose", at)
	if err := m.Save(home); err != nil {
		t.Fatal(err)
	}
	...
	if s := by["diagnose"]; s.From != "bob" || s.LocallyModified {
