...
# Per-caller credentials, for callers that are programs rather than browsers.
# One per line, three whitespace-separated fields, # comments and blanks
# ignored:
#
#   <name>  sha256:<digest of the token>  <os-user>
#   muse    sha256:968726b2020eef753e51fe3e971009e7e105b5d244ce66232d9154519d021c2a  wizard
#
# The file holds the DIGEST of each token and never the token. Mint a token
# where it is going to live (Vault, the caller's own configuration), then
# write its digest here:
#
#   printf %s "$TOKEN" | sha256sum
#
# That is what makes the file safe to have on disk. The services run as an
# ordinary account, so this file must be readable by that account, and
# everything else running as that account reads it too. A digest is worth
# nothing to a reader. A token would be worth every request its caller can
# make.
#
# Own it as root: chown root:<the group the services run as>, then chmod 0640.
# An account that owns the file can append a line, and a line names any
# terminal account on the box, so a file the service account owns is ignored
# rather than trusted.
#
# A request carrying "Authorization: Bearer <token>" is that credential's OS
# user, and sends neither the header above nor the secret above. Issuing is
# adding a line; revoking one caller is deleting its line, which costs no
# other caller anything. The named OS user must already be a terminal account.
#
# Rules that make a line, or the whole file, count for nothing: a presented
# token under 32 characters, a second field that is not a sha256 digest, a
# field that will not parse, a file the world can read or the group can write,
# or a file the service account owns. A skipped line is named by its number in
# the service log. With no credentials at all the feature is off, and a
