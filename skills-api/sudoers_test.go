...
func TestGrantsBinaryIgnoresComments(t *testing.T) {
	if grantsBinary("#   /usr/local/bin/skills-api   re-execs ITSELF as the mapped user\n", "/usr/local/bin/skills-api") {
		t.Error("a comment mentioning the binary must not count as a grant")
	}
	if !grantsBinary("wizard ALL=(bob) NOPASSWD: /usr/bin/tmux, /usr/local/bin/skills-api\n", "/usr/local/bin/skills-api") {
